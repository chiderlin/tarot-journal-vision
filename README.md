...
1. 數據特徵提取：將感性文字「量化」
目前的塔羅日記多半是散亂的文字。要轉換成指數，你需要利用 NLP（自然語言處理） 進行情緒標註：
...
這是你練習技術的重點。你可以將每一篇日記轉化為 Embedding（嵌入向量）。

為什麼要向量化？ 因為向量能捕捉「語義相關性」。

應用場景：當使用者今天寫下「我覺得透不過氣」，系統可以透過向量檢索（Vector Search）發現這跟三個月前他「面臨工作瓶頸」時的語義特徵非常相似，從而識別出週期的壓力模式。

儲存：建議使用 Pinecone、Milvus 或 pgvector (PostgreSQL 擴充)，這對後端工程師來說是非常加分的技能。
