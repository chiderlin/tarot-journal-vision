    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={() => onDateSelect(selectedDate || new Date())}>
                  ...
                  <AccordionItem value={entry.id} key={entry.id}>
                    <AccordionTrigger>
                      <div className="flex justify-between items-center w-full pr-4">
                        <p className="font-semibold truncate text-left">{entry.title}</p>
                        <div className="flex gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                          <Button variant="ghost" size="icon" onClick={() => onEntryEdit(entry)} className="h-8 w-8">
                            <Edit className="w-4 h-4" />
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => onEntryDelete(entry.id)} className="h-8 w-8 text-destructive hover:text-destructive">
