...
serve(async (req) => {
  ...
  try {
    ...
    if (!cards || cards.length === 0) {
      return new Response(
        JSON.stringify({ error: '請先在內容中添加塔羅牌標籤（例如：#fool, #magician）' }),
...
請用溫暖、專業且易懂的語言，給出約200-300字的解析。`;

    const userPrompt = `抽到的牌：${cards.join(', ')}
分類：${category} (${categoryContext[category as keyof typeof categoryContext] || '綜合運勢'})
